...
the model), or `send_file` (a path — its first line is read locally the same
way). `send_env`/`send_file` exist for exactly one reason: a literal `send`
means the actual secret has to be generated by the model to appear in the
tool call at all — round-tripping it through the model's own context and
Anthropic's API twice (once in, once out) even though `secret: true` keeps it
out of the RETURNED transcript afterward. Naming an env var or a file instead
means the model only ever sees/writes a NAME, and the real value is resolved
by this module's own Python code, on this machine, never touching a prompt
or a completion. Same `token_env`/`api_key_env`-name-not-value pattern this
project's own `config.toml` already uses everywhere else for secrets,
applied here to interactive credential prompts specifically. Always treated
as secret for transcript purposes regardless of the `secret` field — there
is no scenario where naming an env var or file and then printing its
...
TOOLS = [
    {
        ...
        "description": (
            ...
            "token specifically, use a step's `send_env`/`send_file` instead of "
            "`send` — the real value is read locally from an environment "
            "variable or a file and never has to be written into this call at "
            "all, unlike a literal `send`, which the user has to tell you and "
            "you have to write down to use. Returns JSON with the terminal "
                        ...
                        "properties": {
                            ...
                            "send": {
                                ...
                                "description": (
                                    ...
                                    "`send_file` is required per step. Use "
                                    "this for non-secret input (a username, "
                                    "'yes', a menu choice) — for a password "
                                    "or token, prefer `send_env`/`send_file` "
                                    "so the real value never has to be "
                                    "written into this call at all."
                                ),
                            },
                            "send_env": {
                                "type": "string",
                            ...
                            "send_file": {
                                "type": "string",
                                "description": (
                                    "Path to a file whose first line is "
                                    "read locally and sent, never written "
                                    "into this tool call — the file-based "
                                    "equivalent of `send_env`, for a "
                                    "credential already stored as a file "
                                    "(e.g. `~/.secrets/some_token`) rather "
                                    "than an env var. Same always-secret "
                                    "transcript treatment and same "
                                    "fail-fast-before-spawning behavior on "
...
def _resolve_reply(step: dict) -> tuple[str, bool, str | None]:
    ...
    Exactly one of `send`/`send_env`/`send_file` must be present — anything
    else (zero, or more than one) is a caller error, returned as `error`
    rather than silently guessing which one was meant. `send_env`/
    `send_file` always come back with `is_secret=True`; a literal `send`
    respects the step's own `secret` field (default `False`), matching this
    tool's existing behavior for that case unchanged.
    """
    sources = [k for k in ("send", "send_env", "send_file") if k in step]
    if len(sources) == 0:
        return "", False, "must include one of `send`, `send_env`, `send_file`"
    if len(sources) > 1:
        return "", False, f"only one of `send`/`send_env`/`send_file` allowed, got {sources}"
    ...
    path = str(step["send_file"])
    try:
        with open(os.path.expanduser(path), encoding="utf-8") as f:
            first_line = f.readline().rstrip("\r\n")
    except OSError as e:
        return "", False, f"could not read send_file {path!r}: {e}"
